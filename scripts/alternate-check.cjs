...
(async()=>{
 ...
 for(const mobile of [true,false]){
  ...
  await gesture();
  await page.waitForFunction(()=>!window.__walkthroughDebug.playing&&window.__walkthroughDebug.frame===277,{},{timeout:12000});
  const elapsed=Date.now()-started;assert.ok(elapsed>=9000&&elapsed<11000,`Part duration ${elapsed}`);
  await page.waitForTimeout(700);assert.equal(await page.evaluate(()=>window.__walkthroughDebug.frame),277);
  await page.screenshot({path:`test-results/alternate-${mobile?'mobile':'desktop'}.png`});
  await gesture();await page.waitForFunction(()=>window.__walkthroughDebug.partEnd===480&&window.__walkthroughDebug.playing);
