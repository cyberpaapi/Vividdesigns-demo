...
(async()=>{
 ...
 for(const mobile of [true,false]){
  ...
  await gesture();
  await page.waitForFunction(()=>!window.__walkthroughDebug.playing&&window.__walkthroughDebug.frame===56,{},{timeout:12000});
  const elapsed=Date.now()-started;assert.ok(elapsed>=1600&&elapsed<2600,`Part duration ${elapsed}`);
  await page.waitForTimeout(700);assert.equal(await page.evaluate(()=>window.__walkthroughDebug.frame),56);
  assert.equal(await page.evaluate(()=>window.__walkthroughDebug.sourceFrame(window.__walkthroughDebug.frame)),118);
  ...
  await gesture();await page.waitForFunction(()=>window.__walkthroughDebug.partEnd===199&&window.__walkthroughDebug.playing);
  ...
  await page.waitForFunction(()=>window.__walkthroughDebug.frame===199);
  await gesture();
  ...
  await gesture(false);
  await page.waitForFunction(()=>window.__walkthroughDebug.playing&&window.__walkthroughDebug.partEnd===199);
  await page.waitForFunction(()=>!window.__walkthroughDebug.playing&&window.__walkthroughDebug.frame===199);
  await page.locator('[data-chapter="4"]').click();
  await page.waitForFunction(()=>window.__walkthroughDebug.frame===597);
  await gesture();
  await page.waitForFunction(()=>!window.__walkthroughDebug.playing&&window.__walkthroughDebug.frame===729);
  assert.equal(await page.evaluate(()=>window.__walkthroughDebug.sourceFrame(window.__walkthroughDebug.frame)),797);
  await page.waitForTimeout(400);
  assert.equal(await page.evaluate(()=>window.__walkthroughDebug.frame),729);
